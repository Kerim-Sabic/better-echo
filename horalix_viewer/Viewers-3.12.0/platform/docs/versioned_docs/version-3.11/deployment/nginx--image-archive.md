...
summary: Tutorial for setting up Horalix Viewer with Nginx and PACS (Orthanc or DCM4CHEE), using Docker for a production-ready system with reverse proxy configuration to securely handle medical imaging data, including installation steps and troubleshooting tips.
---
...
Horalix Viewer and its medical imaging data. This post covers one of many potential
...
- All static resources for Horalix Viewer are served up by `nginx` when a matching
...
The Horalix Viewer's configuration is imported from a static `.js` file. The
...
| ----------------- | --------------------------------- | ------------------------------------------- |
| Horalix Viewer       | [dockerfile][dockerfile]          | You're reading them now!                    |
...
You can follow the similar steps above to run Horalix Viewer with Dcm4chee PACS.
