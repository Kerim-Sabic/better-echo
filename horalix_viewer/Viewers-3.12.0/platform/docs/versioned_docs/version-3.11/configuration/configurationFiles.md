...
summary: Comprehensive guide to configuring Horalix Viewer, including data sources, environment variables, advanced options like study prefetching, and detailed explanations of configuration parameters for performance optimization and feature customization.
---
...
Horalix Viewer has data for several studies and their images. You didn't add this
...
- **Avoids Transcoding**: Transcoding (converting from one transfer syntax to another) can be a resource-intensive process.
 Since the Horalix Viewer supports all transfer syntaxes, it is fine to accept any transfer syntax (transfer-syntax=*).
