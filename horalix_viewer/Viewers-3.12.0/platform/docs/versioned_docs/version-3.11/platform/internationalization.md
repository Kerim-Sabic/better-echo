...
Horalix Viewer already sets a main
[I18nextProvider](https://react.i18next.com/latest/i18nextprovider) connected to
the shared i18n instance from `@ohif/i18n`, all extensions inside Horalix Viewer
...
Horalix Viewer accepts a query param called `lng` in the url to change the
language.
...
We have integrated `i18next` into the Horalix Viewer and hooked it up with Locize
