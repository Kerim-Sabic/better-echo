...
id: technical
title: Technical FAQ
summary: Technical explanations and solutions for Horalix Viewer implementation challenges, including required metadata fields, handling large volumes for MPR/rendering, dynamically loading measurements, customizing series sorting, and addressing common rendering issues.
---
...
## What are the list of required metadata for the Horalix Viewer to work?
...
However, this is the `mapped` cornerstone measurement inside OHIF, and it has additional information such as `geReport` and `source`, which are internal details of Horalix Viewers that you don't need to worry about.
