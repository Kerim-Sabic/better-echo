...
summary: Details how to use the static-wado project to generate and serve optimized static DICOMweb files for enhanced Horalix Viewer performance, with step-by-step instructions for installation, file generation, and viewer configuration.
---


# Static DICOMweb Files for Enhanced Performance

This section describes how to generate and serve static DICOMweb files, significantly improving the performance of your Horalix Viewer setup. These files are pre-processed and compressed, minimizing storage space and reducing serving time to the bare minimum (disk read and HTTP stream write).
...
## Running Horalix Viewer with Static Data

1. **Use the `local_static.js` Configuration:**

    Start the Horalix Viewer in development mode using the provided `local_static.js` configuration file:
...
2. **Configuration Details:**
    ...
    :::info
    If you change the port or output directory when running the `static-wado-webserver`, you **must** also update the `qidoRoot` and `wadoRoot` values in your `local_static.js` configuration file accordingly to ensure the Horalix Viewer can access the data.
    :::
