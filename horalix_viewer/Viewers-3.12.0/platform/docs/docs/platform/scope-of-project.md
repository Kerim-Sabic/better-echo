...
summary: Overview of the Horalix Viewer's scope as a web-based medical imaging viewer that functions as a "dumb client," explaining its capabilities as a Progressive Web Application (PWA), the separation of data from the viewer, and clarification about offline capabilities.
---
# Scope of Project

The Horalix Viewer is a web based medical imaging viewer. This allows it to be used
on almost any device, anywhere. The Horalix Viewer is what is commonly referred to
...
To be more specific, the Horalix Viewer is a collection of HTML, JS, and CSS files.
...
information, the Horalix Viewer's scope **DOES NOT** encompass providing _any_
data; only the configuration necessary to interface with one or more of these
many data sources. The Horalix Viewer's scope **DOES** include configuration and
...
the Horalix Viewer as a desktop application.

_Does the Horalix Viewer work with the local filesystem?_
...
package the Horalix Viewer as an [Electron app][electron].
